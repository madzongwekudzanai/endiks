...
// @router  GET api/auth-seller
// @desc    Get
// @access  Private
router.get('/', sellerAuth, async (req, res) => {
	try {
		const user = await Seller.findById(req.seller.id).select('-password');
		res.json(user);
	} catch (err) {
...
// @router  GET api/auth-seller/verify/:token
// @desc    Verify Seller
// @access  Public
router.get('/verify/:token', verifyToken, async (req, res) => {
	try {
		const user = await Seller.findById(req.id);
		if (user.verified) {
...
// @router  POST api/auth-seller/email
...
// @router  POST api/auth-seller/phone
...
router.post(
	...
	async (req, res) => {
		...
		try {
			let user = await Seller.findOne({ phone });
...
// @router  POST api/auth-seller/forgot/email
// @desc    Seller forgot password using email
// @access  Public
router.post(
	'/forgot/email',
	[check('email', '请输入有效电子邮件').isEmail()],
	async (req, res) => {
		...
		try {
          ...
          <a href="https://endiks.herokuapp.com/seller/auth/reset-password?token=${token}">change password</a>
        `;
				const zhOutput = `
          <h1>你好${user.name}</h1>
          <p>请点击下面的链接重设密码<p/>
          <a href="https://endiks.herokuapp.com/seller/auth/reset-password?token=${token}">更改密码</a>
        `;
...
// @router  POST api/auth-seller/forgot/phone
...
// @router  POST api/auth-seller/reset-password/:token

...
// @router  POST api/sellers/email
// @desc    Register seller using email
// @access  Public
router.post(
	'/email',
	[
	...
	async (req, res) => {
		...
		try {
			...
			const { tradeName, locale, currency, currencyCode, shipping } = invite;
          ...
          <a href="https://endiks.herokuapp.com/seller/auth/sign-in?sellerToken=${token}">Verify account</a>
        `;
				const zhOutput = `
          <h1>帐户验证</h1>
          <p>请点击下面的链接完成注册<p/>
          <a href="https://endiks.herokuapp.com/seller/auth/sign-in?sellerToken=${token}">验证账户</a>
        `;
...
// @router  POST api/sellers/phone
// @desc    Register seller using phone number
// @access  Public
router.post(
	'/phone',
	[
	...
	async (req, res) => {
		...
		try {
			let invite = await Invite.findOne({ phone });
			...
			jwt.sign(payload, jwtSecret, { expiresIn: 360000 }, (err, token) => {
				if (err) throw err;
				const from = 'Endiks';
				const enText = `【Endiks】Endiks business account registration completed`;
				const zhText = `【Endiks】祝贺您的Endiks商业帐户注册成功`;
				const opts = {
					type: 'unicode',
				};
